...
export default function ProductFilters({
  ...
  const selectedTagSlugs = selectedTags ? selectedTags.split(',') : [];
  ...
  const content = (
    <div className="space-y-6">
      ...
      {/* Tags — its own section, sibling of Categories/Metal, not nested inside either */}
      {tags.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-semibold text-primary">برچسب‌ها</h3>
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-xs transition-colors ${selectedTagSlugs.includes(tag.slug)
                  ? 'border-accent bg-accent-subtle text-accent'
                  : 'border-border-default text-primary-muted hover:bg-surface-sunken'
                  }`}
              >
                {tag.name}
              </button>
